function [vignetting_fit] = fit_vignetting_falloff(filename, FOV);
    ...
    bg = imread(filename)'; % Transpose so X is on index 1
    
    PixelSize = FOV / size(bg, 1);
    
    % Use some centre portion of the image...
    CROP = 40;
    bg = bg((1+CROP):(end-CROP), (1+CROP):(end-CROP));
    %figure(12);
    %subplot(2,1,1);
    %imagesc(bg');
    ...
    for i = 1:size(bg, 1) % X axis on index 1
        for j = 1:size(bg, 2) % Y axis on index 2
            c = c + 1;
            x(c) = i;
            y(c) = j;
            z(c) = bg(i,j);
