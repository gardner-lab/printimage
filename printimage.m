...
% Last Modified by GUIDE v2.5 20-Dec-2019 16:19:00
...
menu_calibrate_vignetting_compensation_save_baseline = uimenu(menu_calibrate, 'Label', 'Save vignetting image', 'Callback', @calibrate_vignetting_save_baseline_Callback);
menu_calibrate_vignetting_compensation = uimenu(menu_calibrate, 'Label', 'Calibrate laser power compensation', 'Callback', @calibrate_vignetting_slide);
menu_restore_last_vignetting_compensation = uimenu(menu_calibrate, 'Label', 'Restore last laser power compensation', 'Callback', @calibrate_vignetting_restore);
menu_clear_vignetting_compensation = uimenu(menu_calibrate, 'Label', 'Clear laser power compensation', 'Callback', @clear_vignetting_compensation_functions);
...
UpdateBounds_Callback(hObject, eventdata, handles);
update_meta_spot(hObject, eventdata, handles);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Laser power compensation controls
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function push_lasermap_Callback(hObject, eventdata, handles)
%Initial attempt to implement absolute laser power control
...
pows = [0, 7.56, 9.76, 18.1, 27, 45.7, 69.6, 94.2, 144.1, 196.2, 249.8, 305, 361.8, 422.8, 455.6, 489.1, 526.6, 543.1, 585.1]; %laser power at objective (in mW)
...
%Display mapping and assemble into a 2D array
axes(handles.axes3); cla
plot(pers, pows, '.-k');
set(gca, 'TickDir', 'out', 'Box', 'off', 'XTick', [0:.5:1], 'YTick', [0:100:500]);
xlim([0,1]); ylim([0,500])
xlabel('Laser %');
ylabel('Laser mW');
...
%Add to carried variable
STL.calibration.perpow = perpow;
STL.print.pp = true;

%Change indicator and permisions
set(handles.check_laserwatts, 'BackgroundColor', 'g')

disp('%-to-mW mapping completed. Change control mode as desired.')

guidata(hObject, handles);
end

function check_laserpercent_Callback(hObject, eventdata, handles)
%Don't allow uncheck function; must select another checkbox to unselect
if ~get(handles.check_laserpercent,'Value')
    set(handles.check_laserpercent,'Value',1);
end

%Reset the linked checkboxes
set(handles.check_laserwatts,'Value',0);
...
function check_laserwatts_Callback(hObject, eventdata, handles)
%If laser mapping isn't complete, this check shouldn't do anything
global STL;
if ~isfield(STL.print, 'pp') || ~STL.print.pp
    error('%-to-mW mapping is not complete. Create mapping and retry.')
    return
end

%Don't allow uncheck function; must select another checkbox to unselect
if ~get(handles.check_laserwatts,'Value')
    set(handles.check_laserwatts,'Value',1);
end

%Reset the linked checkboxes
set(handles.check_laserpercent,'Value',0);

%Update label
set(handles.text4, 'String', 'mW');
